...
def predictAndEvaluate(posTagger, testData):
    ...
    for sent_with_tags in test_data:
        ...
        predicted_tag_pairs = posTagger.tag(sent)
    ...
    return tagAccuracyDict, allTagsTotalDict, confusionMatrix

train_data, test_data = prepareData()

bigramTagger = BigramTagger(train_data)

tagAccuracyDict, allTagsTotalDict, confusionMatrix = predictAndEvaluate(bigramTagger, train_data)
accuracyDict = {key: float(tagAccuracyDict[key])/float(allTagsTotalDict[key]) for key in tagAccuracyDict}
print accuracyDict